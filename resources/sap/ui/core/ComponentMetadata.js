...
sap.ui.core.ComponentMetadata.prototype._loadIncludes=function(){var I=this.getIncludes();if(I&&I.length>0){var t=this;var l=this.getLibraryName();jQuery.each(I,function(i,f){if(f.match(/\.css$/i)){var c=sap.ui.resource(l,f);jQuery.sap.log.info("Component \""+t.getName()+"\" is loading CSS: \""+c+"\"");jQuery.sap.includeStyleSheet(c)}else{var m=f.match(/\.js$/i);if(m){var p=l.replace(/\./g,'/')+(f.slice(0,1)==='/'?'':'/')+f.slice(0,m.index);jQuery.sap.log.info("Component \""+t.getName()+"\" is loading JS: \""+p+"\"");jQuery.sap._requirePath(p)}}})}};
